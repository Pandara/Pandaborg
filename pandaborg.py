...
class ircInterface:
  ...
  channel = "#kawayui" # Channel
  channel_pass = "yuiaki27" #channel pass
  ...
  def __init__(self, inServer = server, inChannel = channel, inChanPass = channel_pass, inNick = botnick, nsIn = nickServPass):
    self.server = inServer
    self.channel = inChannel
    self.channel_pass = inChanPass
    self.botnick =  inNick
    ...
    while self.connected: 
      ...
      if ircmsg.find("Password accepted - you are now recognized") != -1:
        self.joinchan(self.channel, self.channel_pass) # Join the channel using the functions we previously defined
        self.joinchan("#hajime", "") 
        self.joinchan("#nyaa-nyaa", "")
        self.joinchan("#nyaa-ni", "")
        self.joinchan("#nyaa-lewd", "")
        self.joinchan("#navypandas", "")
        self.joinchan("#bots", "")
...
#        self.sendaction(origin, "hugs " + name + "~")
  ...
  def processConsoleReq(self, command): ## TODO: "JOINCHAN, LEAVECHAN, SENDMSG, QUIT, SENDALL, SENDACTION"
    ...
    if action[0].find("HELP") != -1:
      responce = "Avalible Commands: HELP"
